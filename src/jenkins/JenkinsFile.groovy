			...
			steps{
				script{
					docker.withRegistry('https://gcr.io', 'gcr:cosmonaut-registry-sa') {
						docker.image("gcr.io/cosmonaut-299500/cosmonaut/admin:$tag").push(tag)
					}
				}
			}
		}
			...
			steps{
				 script{
					 withCredentials([file(credentialsId: 'cosmonaut-k8s-sa', variable: 'serviceaccount')]) {
					   ...
					   sh "kubectl get pods"
					   sh "sed -i 's/\$TAG/$tag/' src/jenkins/cosmonaut_admin_deployment.yml"
					   sh "cat src/jenkins/cosmonaut_admin_deployment.yml | kubectl apply -f -"
					 }
				 }
			}
		}
