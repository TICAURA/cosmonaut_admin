    ...
    stage("Compilacion") {
     ...
     stage("Push de imagen") {
        container('docker-registry'){
            script{
                sh 'ls /secret/ -la'
				sh "/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --cache=true --destination=gcr.io/cosmonaut-uat/cosmonaut/admin:$tag"
			}
        }
     }
     stage('Deploy to Kubernetes') {
        container('container-registry'){
            script{
		      ...
		      sh "sed -i 's/\$TAG/$tag/' src/jenkins/cosmonaut_admin_deployment-qa.yml"
		      sh "cat src/jenkins/cosmonaut_admin_deployment-qa.yml | kubectl --insecure-skip-tls-verify apply -f -"
		     }
        }
     }
     }     
